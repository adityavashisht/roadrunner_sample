...
/**
 * Custom Extractor for processing response from API
 *
 *
 *
 *
 */
public class GetAirportExtractor implements SourceExtractor {
    ...
    public Object extractData(Source source) throws IOException, TransformerException {
        ...
        if (nodeList.getLength() != 0) {
            ...
            for (int index = 0; index <= length; index++) {
                ...
                if (value != null && "Airport".equals(value.getNodeName())) {
                    NamedNodeMap attributeMap = value.getAttributes();
                    String longName = attributeMap.getNamedItem("Longname").getNodeValue();
                    String name = attributeMap.getNamedItem("Name").getNodeValue();
                    KeyValueDropDown dropDown = new KeyValueDropDown(name,longName);
